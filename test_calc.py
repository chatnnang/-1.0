...
print("TOP 5:")
for i, x in enumerate(top50[:5]):
    print(f"{i+1}. {x['title']} [{x['diff']}] VF: {x['vf']} (Level: {x['level']}, Score: {x['score']})")

print("RANKS 45-50:")
for i, x in enumerate(top50[45:]):
    print(f"{i+46}. {x['title']} [{x['diff']}] VF: {x['vf']} (Level: {x['level']}, Score: {x['score']})")
